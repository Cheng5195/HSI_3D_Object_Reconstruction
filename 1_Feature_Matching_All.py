...
class Config(CommonConfig):
    """Configuration parameters management class (for all bands matching)"""
...
class ImageLoader:
    ...
    def load_hyperspectral_bands(folder_path):
        """Load all hyperspectral image bands"""
        bands = []
        start = Config.HSI_PARAMS['start_wavelength']
        end = Config.HSI_PARAMS['end_wavelength']
        step = Config.HSI_PARAMS['step']
        wavelengths = range(start, end, step)
    ...
    def load_all_data(cls, object_name, num_frames):
        ...
        for frame_num in range(1, num_frames + 1):
            # Load hyperspectral image bands
