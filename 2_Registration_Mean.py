...
def load_edge_info(object_name, i, j):
    data = np.load(f'transformation/Mean/{object_name}/transformation_{i}_{j}.npz')
    return {
        'source': i,
        'target': j,
...
def main():
    ...
    results_dir = f'results/Mean/{object_name}'
    ...
    #                              f"{results_dir}/1_initial_alignment_Mean.ply")
    ...
    #     f"{results_dir}/2_after_global_optimization_Mean.ply"
    # )
    ...
    visualize_and_save_point_cloud(
        ...
        f"{results_dir}/3_final_result_Mean.ply"
    )
