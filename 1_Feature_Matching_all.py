...
def load_hyperspectral_bands(folder_path):
    """Load specified bands (500nm-650nm) from hyperspectral image"""
    bands = []
    # Generate wavelength list from 500nm to 650nm with 5nm intervals
    wavelengths = range(500, 655, 5)
...
def ransac_3d_rigid_transform(pts1, pts2, threshold=0.015, max_iters=1000):
    ...
    for _ in range(max_iters):
        sample_idx = np.random.choice(len(pts1), 5, replace=False)
...
def main():
    ...
    output_dir = f'transformation/all/{object_name}'
    ...
    for i in range(num_images):
        ...
        # Load all bands from hyperspectral image
        gray_folder = f'{object_name}/HSI/400_800nm/imge{frame_num}'
