...
def load_edge_info(object_name, i, j):
    data = np.load(f'transformation/rgb/{object_name}/transformation_{i}_{j}.npz')
    return {
        'source': i,
        'target': j,
...
def main():
    ...
    results_dir = f'results/rgb/{object_name}'
    ...
    visualize_and_save_point_cloud(
        ...
        f"{results_dir}/3_final_result_rgb_no_icp.ply"
    )
